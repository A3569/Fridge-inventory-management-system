...
API_BASE_URL = 'http://localhost:3000/api/v1'  # Update this with your API server URL
...
def index(request):
    ...
    user, created = User.objects.get_or_create(
        ...
        defaults={
            'is_active': True,
            'is_staff': False,  # Remove staff status
...
def api_add_to_inventory(request):
    if request.method == 'POST':
        try:
            ...
            print("Found item type:", item_type)  # Debug print
            ...
            print("Created inventory item:", item)  # Debug print
            ...
            print(f"ItemType not found for barcode: {data.get('itemType')}")  # Debug print
            return JsonResponse({'error': 'Item type not found'}, status=404)
        except KeyError as e:
            print(f"Missing required field: {e}")  # Debug print
            return JsonResponse({'error': f'Missing required field: {e}'}, status=400)
        except Exception as e:
            print(f"Error adding to inventory: {e}")  # Debug print
...
def api_remove_from_inventory(request):
    if request.method == 'DELETE':
        ...
        try:
            ...
            print(f"Error removing from inventory: {e}")  # Add debugging
            return JsonResponse({'error': str(e)}, status=400)
...
def api_add_to_shopping_list(request):
    if request.method == 'PUT':
        ...
        try:
            ...
            print(f"Error adding to shopping list: {e}")  # Add debugging
            return JsonResponse({'error': str(e)}, status=400)
    return JsonResponse({'error': 'Invalid request'}, status=405)
...
def api_remove_from_shopping_list(request):
    if request.method == 'DELETE':
        ...
        try:
            ...
            print(f"Error removing from shopping list: {e}")  # Add debugging
            return JsonResponse({'error': str(e)}, status=400)
    return JsonResponse({'error': 'Invalid request'}, status=405)
...
def api_purchase_item(request):
    if request.method == 'PATCH':
        ...
        try:
            ...
            print(f"Error purchasing item: {e}")  # Add debugging
            return JsonResponse({'error': str(e)}, status=400)
    return JsonResponse({'error': 'Invalid request'}, status=405)